...
float ieee754_to_float(uint8_t byte1, uint8_t byte2, uint8_t byte3,
		uint8_t byte4);
...
int main(void) {
	...
	if (HAL_UART_Transmit(&huart1, txData, sizeof(txData), HAL_MAX_DELAY)== HAL_OK) {
		Print_Raw_Data(txData, sizeof(txData));
		// Receive the response from the energy meter
		if (HAL_UART_Receive(&huart1, rxData, sizeof(rxData), HAL_MAX_DELAY)== HAL_OK) {
			...
			float voltage = ieee754_to_float(byte1, byte2, byte3, byte4);
			printf("Voltage: %f V\n", voltage);
		} else {
		...
		printf("failed to Transmited data ");
	}
